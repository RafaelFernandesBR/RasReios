...
namespace RasReiios
{
    public class TelegramService : IChatService, IDisposable
    {
        ...
        public async Task<bool> SendMessage(long chatId, string? message, int id, Dictionary<string, string>? buttons = null)
        {
            try
            {
                ...
                await _botClient.SendTextMessageAsync(new ChatId(chatId), message, parseMode: ParseMode.MarkdownV2, replyMarkup: GetInlineKeyboard(buttons), replyToMessageId: id);
                return true;
            }
            catch (Exception ex)
            {
